//! Pose-graph loading: types, the [`GraphLoader`] trait, and the G2O/TORO codecs.
//!
//! The pose graph is a distinct data model from the time-ordered
//! [`Trajectory`](crate::trajectory::Trajectory): vertices plus edges with
//! information matrices, no timestamps. It lives in its own module for the same
//! reason the trajectory module does — one home per format family.
...
pub fn load_graph<P: AsRef<Path>>(path: P) -> Result<Graph, IoError> {
    ...
    match extension.to_lowercase().as_str() {
        "g2o" => g2o::G2oLoader::load(path),
        "graph" => toro::ToroLoader::load(path),
        _ => Err(
...
mod tests {
    ...
    fn test_load_simple_graph() -> Result<(), IoError> {
        ...
        let graph = g2o::G2oLoader::load(temp_file.path())?;
    ...
    fn test_duplicate_vertex_error() -> Result<(), io::Error> {
        ...
        let result = g2o::G2oLoader::load(temp_file.path());
    ...
    fn test_toro_loader() -> Result<(), IoError> {
        ...
        let graph = toro::ToroLoader::load(temp_file.path()).map_err(|e| e.log())?;
        assert_eq!(graph.vertices_se2.len(), 2);
