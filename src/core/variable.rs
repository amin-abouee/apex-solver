...
impl VariableType {
    ...
    /// Get the tangent space dimension (degrees of freedom)
    ...
    pub fn minus(&self, other: &VariableType) -> na::DVector<f64> {
        match (self, other) {
            (VariableType::Vector(v1), VariableType::Vector(v2)) => {
                v1 - v2
            },
...
mod tests {
    ...
    fn test_variable_type_so3_operations() {
        ...
        if let VariableType::SO3(result_so3) = result_type {
            ...
            assert!((orig_q.w - result_q.w).abs() > 1e-10 || 
    ...
    fn test_variable_update_values_with_fixed_indices() {
        ...
        let new_values = na::DVector::from_vec(vec![10.0, 20.0, 30.0, 40.0, 50.0, 60.0, 70.0, 80.0]);
        variable.update_values(new_values);
