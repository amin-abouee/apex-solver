//! Integration test for EUCM camera with multi-camera calibration
//!
...
//! - Tests Extended Unified Camera Model's ability to handle wide-angle distortion
//!
//! EUCM Camera Model:
//! - 6 intrinsic parameters: fx, fy, cx, cy, alpha, beta
...
/// Test EUCM camera self-calibration with multi-camera setup.
///
...
///
/// Ground truth camera: 600x400 image, EUCM model with alpha=0.6, beta=1.2
#[test]
fn test_eucm_multi_camera_calibration_200_points() -> TestResult {
    // ============================================================================
    // 1. Ground Truth Setup - 600x400 EUCM Camera
    // ============================================================================

    // EUCM camera parameters for wide-angle lens
    ...
    // For EUCM, use planar wall to ensure all points visible
    ...
    // constraints for recovering projection and distortion parameters.
    ...
    let true_intrinsics = [200.0, 200.0, 300.0, 200.0, 0.6, 1.2];
    ...
    for (cam_idx, observations) in all_observations.iter().enumerate() {
        ...
        let factor: ProjectionFactor<EucmCamera, SelfCalibration> =
...
#[test]
fn test_eucm_3_cameras_calibration() -> TestResult {
    ...
    // Generate calibration points on wall (not hemisphere for EUCM)
    let true_landmarks = generate_wall_calibration_points(20, 10, 0.1, 3.0);
    ...
    // Optimize.
    //
    // 300 iterations, not 100: self-calibration mixes focal lengths (~200),
    ...
    // long way. It gets there — `ParameterToleranceReached` at iteration 194,
    // final cost 1.5e-2. With the old uniform λI this same problem stalled at
    // iteration 29 with a cost of 1.5e3, five orders of magnitude worse, and
