//! Integration test for Eucm camera with SelfCalibration
//!
...
//!
//! The eucm model is the simplest camera model with no distortion,
...
#[test]
fn test_eucm_self_calibration_50_points() {
    // ============================================================================
    // 1. Ground Truth Setup
    // ============================================================================

    // Eucm camera with typical parameters (VGA resolution, ~60° FOV)
    let true_camera = EucmCamera::new(
        480.0, 480.0, 320.0, 240.0,  // fx, fy, cx, cy
        0.6, 1.2  // alpha, beta
    );
    ...
    let noisy_intrinsics = perturb_intrinsics(&[480.0, 480.0, 320.0, 240.0, 0.6, 1.2], 0.04, 789);
    ...
    info!("\n========================================");
    info!("Eucm Self-Calibration Results:");
    info!("========================================");
    ...
    let final_intrinsics = result
        ...
        .to_vector();
    let true_intrinsics = [480.0, 480.0, 320.0, 240.0, 0.6, 1.2];
    ...
    for i in 0..4 {
        ...
        // Eucm should recover intrinsics very accurately (no distortion, well-conditioned)
        assert!(
            param_error < 0.10,
