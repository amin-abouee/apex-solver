//! Integration test for Fov camera with SelfCalibration
//!
...
//!
//! The fov model is the simplest camera model with no distortion,
...
#[test]
fn test_fov_self_calibration_50_points() {
    // ============================================================================
    // 1. Ground Truth Setup
    // ============================================================================

    // Fov camera with typical parameters (VGA resolution, ~60° FOV)
    ...
    let noisy_intrinsics = perturb_intrinsics(&[500.0, 500.0, 320.0, 240.0, 0.8], 0.05, 789);
    ...
    info!("\n========================================");
    info!("Fov Self-Calibration Results:");
    info!("========================================");
    ...
    let final_intrinsics = result
        ...
        .to_vector();
    let true_intrinsics = [500.0, 500.0, 320.0, 240.0, 0.8];
    ...
    for i in 0..4 {
        ...
        // Fov should recover intrinsics very accurately (no distortion, well-conditioned)
        assert!(
            param_error < 0.10,
