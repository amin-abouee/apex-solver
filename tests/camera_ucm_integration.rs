//! Integration test for Ucm camera with SelfCalibration
//!
...
//!
//! The ucm model is the simplest camera model with no distortion,
...
#[test]
fn test_ucm_self_calibration_50_points() {
    // ============================================================================
    // 1. Ground Truth Setup
    // ============================================================================

    // Ucm camera with typical parameters (VGA resolution, ~60° FOV)
    let true_camera = UcmCamera::new(
        490.0, 490.0, 320.0, 240.0,  // fx, fy, cx, cy
        0.7  // alpha
    );
    ...
    let noisy_intrinsics = perturb_intrinsics(&[490.0, 490.0, 320.0, 240.0, 0.7], 0.05, 789);
    ...
    info!("\n========================================");
    info!("Ucm Self-Calibration Results:");
    info!("========================================");
    ...
    let final_intrinsics = result
        ...
        .to_vector();
    let true_intrinsics = [490.0, 490.0, 320.0, 240.0, 0.7];
    ...
    for i in 0..4 {
        ...
        // Ucm should recover intrinsics very accurately (no distortion, well-conditioned)
        assert!(
            param_error < 0.10,
